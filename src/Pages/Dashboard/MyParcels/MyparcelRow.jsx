...
const MyparcelRow = ({booking, bookings, setBookings,index}) => {
    ...
    return (
        ...
        {/* row 1 */}
          <tr >
              ...
              <td > 
                 <Link
                 to={status === 'delivary' || status === 'On The Way' ? '#' : `/dashboard/up/${_id}`}
                 onClick={(e) => {
                if (status === 'delivary' || status === 'On The Way') {
                e.preventDefault();  } }}
                className={`btn btn-outline btn-secondary ${
               (status === 'delivary' || status === 'On The Way') ? 'opacity-50 cursor-not-allowed' : ''}`}>
                Update
              </Link>
             </td>
              <td>
                <button onClick={() => haldelDelete(_id)} className="btn btn-outline btn-warning"   disabled={status === 'delivary' || status === 'On The Way'}>Cancel</button>
             </td>
              <td>
               <Button
  onClick={open}
  disabled={status === 'pending' || status === 'On The Way'} // Disable for pending and On The Way status
  className={`rounded-md btn-outline py-2 px-4 text-sm font-medium focus:outline-none ${status === 'pending' || status === 'On The Way' ? 'bg-gray-400 cursor-not-allowed' : 'bg-secondary text-white hover:bg-black/30 focus:outline-1 focus:outline-white'}`}
>
  Review
</Button>
                </td>
